...
public class JarAnalyzer {
	...
	public void parseWholeLib(int versionTypeId,String decompiledLibPath) {			
		parseFile(versionTypeId,decompiledLibPath,decompiledLibPath);
//		JsonFileUtil.save(SAVE_PATH + versionTypeId+".txt", clazzs);
	}
...
//				System.out.println(constructor);
	...
	public void parseLibById(int typeId) {
	...
	public static void getPaths() {
		...
		for(int i = 301;i<=2600;i++ ) {
			ResultSet rs = DB.query("SELECT * FROM `version_types` where `type_id`=" + i);
			try {
				while (rs.next()) {
					String jackage_url = rs.getString("jar_package_url");
					if(jackage_url.endsWith(".jar")) {
//						String path = "F:/GP/decompile/"+jackage_url.substring(0, jackage_url.length()-4)+"_decompile";
//						File file = new File(path);
//						if(!file.exists()) {
//							System.out.println("package : " + path +"  not exists");
//							return;
...
//						parseWholeLib(typeId,path);
						JSONObject obj = new JSONObject();
						obj.put("id", i);
						obj.put("path", jackage_url.substring(0, jackage_url.length()-4)+"_decompile");
						paths.add(obj);
					}
				}
			} catch (SQLException e) {
		...
		JsonFileUtil.save(SAVE_PATH +"path.txt", paths);
	}
	public static void parseLib() {
		String whole = JsonFileUtil.readJsonFile(INPUT_FILE_PATH+"path.txt");
		JSONArray array = JSONArray.fromObject(whole);						
		for (int a = 0; a < array.size(); a++) {
			JSONObject obj = array.getJSONObject(a);
			int id = obj.getInt("id");
			String path = obj.getString("path");
			path = DECOMPILED_LIB_PATH +path;
			File file = new File(path);
			if(!file.exists()) {
				System.out.println("package : " + path +"  not exists");
				return;
